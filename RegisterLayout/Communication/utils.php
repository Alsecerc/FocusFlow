...
// function SendFriendRequest($ContactID, $status) {
//     global $user_id;

//     // Get FriendID from contact table
//     $FriendID = Query("SELECT FriendID FROM contact WHERE id = ?", "i", $ContactID, "Friend ID not found", "single", "SELECT", true);
//     if (!$FriendID) {
...
//     // Get UserFriendID (checking both directions)
//     $UserFriendID = Query("SELECT friend_id FROM friends WHERE id = ? AND user_id = ?", "ii", [$FriendID['FriendID'], $user_id], "Friend ID not found", "single", "SELECT", null);
//     if (!$UserFriendID) {
//         $UserFriendID = Query("SELECT user_id FROM friends WHERE id = ? AND friend_id = ?", "ii", [$FriendID['FriendID'], $user_id], "Friend ID not found", "single", "SELECT", true);
//     }
//     if (!$UserFriendID) {
//         echo json_encode(['status' => 'error', 'message' => 'Friend ID not found']);
//         exit;
//     }

//     // Check if a friend request already exists
//     $CheckIfFriendRequestExist = Query(
//         "SELECT 1 FROM friendrequest WHERE user_id = ? AND friend_id = ?",
//         "ii",
//         [$user_id, $UserFriendID['friend_id']],
...
//     if (!$CheckIfFriendRequestExist) {
//         // Create a new friend request
//         $SendFriendrequest = Query(
//             "INSERT INTO friendrequest (user_id, friend_id, Status) VALUES (?, ?, ?)",
//             "iis",
//             [$user_id, $UserFriendID['friend_id'], $status],
...
//         $SendFriendrequest = Query(
//             "UPDATE friendrequest SET Status = ? WHERE friend_id = ? AND user_id = ?",
//             "sii",
//             [$status, $UserFriendID['friend_id'], $user_id],
